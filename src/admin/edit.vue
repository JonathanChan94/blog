<template>
  <div class="add">
    ...
    <div class="add-form-submit-container">
      <div class="add-form-submit" @click="submit">更新文章</div>
...
export default {
  ...
  methods: {
    getData () {
      this.$store.dispatch('getArticleTotal', this.articleId).then(res => {
        this.title = res.res.title;
        this.date = this.dateForm(res.res.date);
        this.tags = res.res.tags;
        this.introMd = res.res.introMd;
        this.contentMd = res.res.contentMd;
      }).catch(err => {})
    },
    dateForm (value) {
      if (!value) return "";
